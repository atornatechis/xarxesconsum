...
if($contadorPedidos>0){
	...
	for($i=0;$i<count($pedidos);$i++){
		...
		if($pedidos[$i]['id_xarxa']!=$pedidos[$i+1]['id_xarxa'] || $pedidos[$i]['id_productorx']!=$pedidos[$i+1]['id_productorx']){
			...
			$sqlInsertEnvio='INSERT INTO envio (id_xarxa,nombre_xarxa,email_xarxa,email_admin,id_productorx,nombre_productorx,email_productorx,fecha_envio,asunto,cuerpo,enviado) VALUES ("'.$pedidos[$i]['id_xarxa'].'","'.$pedidos[$i]['nomXarxa'].'","'.$emailXarxa.'","'.$emailAdmin.'","'.$pedidos[$i]['id_productorx'].'","'.$pedidos[$i]['nombre_productorx'].'","'.$emailProductorx.'",NOW(),"'.htmlentities(addslashes($asunto),ENT_COMPAT,'ISO-8859-1').'","'.htmlentities(addslashes($cuerpo),ENT_COMPAT,'ISO-8859-1').'","0")';
...
if($resultEnvio=mysql_query($sqlEnvio)){
	...
	while($filaEnvio=mysql_fetch_object($resultEnvio)){
		...
		$nombreXarxa=utf8_decode($filaEnvio->nombre_xarxa);
