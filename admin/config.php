...
if ( isset($_POST["accept2"]) ){
	$horaLimite=$_POST['horalimit'].':'.$_POST['minutoslimit'].':00';
	//configuracio xarxa
	$sqlConfig="SELECT nom_xarxa FROM config";
	if(mysql_num_rows(mysql_query($sqlConfig))){
		$con_upd="UPDATE config SET nom_xarxa='".mysql_real_escape_string($_POST['nomxarxa'])."', dia_venda='".$_POST['diavenda']."', dia_limit='".$_POST['dialimit']."', hora_limit='".$horaLimite."', descripcio='".$_POST['textweb']."', limit_comanda_saldo='".$_POST['limit_comanda_saldo']."'";
		$res_upd=mysql_query($con_upd) or die(mysql_error());
		if($res_upd)
			$_SESSION["nom_xarxa"]=$_POST['nomxarxa'];
	}else{
		$sqlInsertCorreo="INSERT INTO config (nom_xarxa, dia_venda, dia_limit, hora_limit, descripcio, limit_comanda_saldo) VALUES ('".mysql_real_escape_string($_POST['nomxarxa'])."', '".$_POST['diavenda']."', '".$_POST['dialimit']."', '".$horaLimite."', '".$_POST['textweb']."', '".$_POST['limit_comanda_saldo']."')";
		$resInsertCorreo=mysql_query($sqlInsertCorreo) or die(mysql_error());
		if($resInsertCorreo)
			$_SESSION["nom_xarxa"]=$_POST['nomxarxa'];
	}
...
?>
				<table border="0" cellpadding="3" cellspacing="0" align="center" width="90%">
					...
					<tr>
						...
						<td>
							<select name="horalimit" class="required">
<?php
								$horaCompleta=array();
								$horaCompleta=explode(':',$xarxa->hora_limit);
								for($hora=0;$hora<=23;$hora++){
									$horaSelect=$hora;
									if($hora<10)
										$horaSelect='0'.$hora;
?>
									<option value="<?php echo $horaSelect; ?>" <?php if($horaSelect==$horaCompleta[0]) echo 'selected="selected"'; ?> ><?php echo $horaSelect; ?></option>
<?php
								}
?>
							</select>
							:
							<select name="minutoslimit" class="required">
<?php
								for($minutos=0;$minutos<=55;$minutos+=5){
									$minutosSelect=$minutos;
									if($minutos<10)
										$minutosSelect='0'.$minutos;
?>
									<option value="<?php echo $minutosSelect; ?>" <?php if($minutosSelect==$horaCompleta[1]) echo 'selected="selected"'; ?> ><?php echo $minutosSelect; ?></option>
<?php
								}
?>
							</select>
