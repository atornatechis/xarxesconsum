		...
		case "add":
			...
			if(isset($_POST['correu']) && $_POST['correu']!='' && isset($_POST['nom']) && $_POST['nom']!=''){
				...
				if(!mysql_fetch_object($resultEmail)){
					...
					smtpmailer($to,$subject,$body);
					$fet=CORRECTO_ANYADIR_U_CONSUMO.': '.$_SESSION['nom_xarxa'];
				}else{
					$fet=ERROR_EXISTE_EMAIL.': '.$_SESSION['nom_xarxa'];
				}
			}else{
				$fet=ERROR_DATOS_INTRODUCIDOS;
			}
...
<?php
			...
			if(isset($_GET['n']) && $_GET['n']!='')
				$n=addslashes(strip_tags($_GET['n']));
			else
				$n=0;
			$cantidadRegistros=0;
			//------------------------------------------------
			$sqlCantidadUsuarixs='SELECT COUNT(*) FROM unitat_c';
			$resultCantidadUsuarixs=mysql_query($sqlCantidadUsuarixs);
			if($resultCantidadUsuarixs!=false){
				$numUsuarixs=mysql_fetch_row($resultCantidadUsuarixs);
				if($numUsuarixs[0]>0){
					$cantidadRegistros=$numUsuarixs[0];
				}
			}
			$sqlUsuarixs='SELECT id,nom,n_caixa,correu FROM unitat_c ORDER BY n_caixa ASC LIMIT '.$n.','.$rango;
			$resultUsuarixs=mysql_query($sqlUsuarixs) or die(mysql_error());
			//------------------------------------------------
			//FIN PAGINADOR
			//------------------------------------------------
			while($unitat_c=mysql_fetch_object($resultUsuarixs)){
...
<?php
					...
					if($n!=0){
?>
						<a href="usuaries.php?n=<?php echo $n-$rango ?>" title="<?php echo ANTERIOR; ?>"><input type="button" value="<?php echo $simboloAnt; ?>" /></a>
<?php
					}
...
?>
							<a href="usuaries.php?n=<?php echo $rango*$pag ?>"><input type="button" value="<?php echo ($pag+1); ?>" /></a>
<?php
						}
					}
					if(($n+$rango)<$cantidadRegistros){
?>
						<a href="usuaries.php?n=<?php echo $n+$rango ?>" title="<?php echo SIGUIENTE ?>"><input type="button" value="<?php echo $simboloSig; ?>" /></a>
<?php
					}
					//-------------------------
