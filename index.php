				...
				<table style="padding: 2px 0;" align="right" width="90%">
					<tr>
						<td width="35%" align="right">
							<label for="usuari"><?php echo EMAIL; ?>: <input style="width: 50%;" type="text" class="required" size="16" maxlength="50" name="usuari" /></label>
						</td>
						<td width="35%" align="right">
							<label for="pass"><?php echo PASSWORD; ?>: <input style="width: 50%;" type="password" class="required" size="16" maxlength="50" name="pass" /></label>
						</td>
...
<?php
									for( $j = 0; $j < count($nom_xarxa); $j++ ){
										//Recortamos el nombre del grupo si es largo para que no se descuadre la tabla
										$longMaxNombre=15;
										$nombreGrupo=html_entity_decode($nom_xarxa[$j]);
										if(strlen($nombreGrupo)>$longMaxNombre)
											$nombreGrupo=substr($nombreGrupo,0,$longMaxNombre)."...";
										echo '<option value="'.$id_xarxa[$j].'">'.$nombreGrupo.'</option>';
									}
?>
								</select>
							</label>
